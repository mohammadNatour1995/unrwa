    ...
    errorPlacement: function (error, element) { // render error placement for each input type
        ...
        if (help.length > 0) {
            ...
            if (element.closest('.bootstrap-select').length > 0) {     //Bootstrap select
                ...
                if (element.closest('.input-group').find("div.input-group-append").length > 0) {//when the input contains Icon
                    //console.logconsole.log("element" + element.attr("class"));
                ...
                } else if (element.hasClass("kt-select2")) {
                    element.closest('.kt-select2').parent().find('> span').after(error);
                }
                else {
                    element.after(error);
                }
